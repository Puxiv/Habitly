...
struct DashboardView: View {
    ...
    private var notesVM: NotesViewModel { NotesViewModel(modelContext: modelContext) }
        ...
        return LazyVGrid(columns: columns, spacing: 12) {
            ...
            if lang.moduleNotes {
                gridCard(
                    icon: "note.text",
                    iconBg: Color.purple,
                    title: lang.dashNotesTitle,
                    value: noteItems.isEmpty ? "—" : "\(notesVM.activeCount(from: noteItems))",
                    subtitle: noteItems.isEmpty ? lang.dashNoNotesYet : recentNotesSubtitle,
                    tab: 3
                )
            }
